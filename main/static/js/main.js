        ...
        for (const [i, data] of leaderboard.entries()) {
           ...
           <tr class="group hover:bg-white/[0.02] transition-colors relative ">
            <th class="flex justify-center">
              <div class="relative size-13 flex items-center justify-center ${rowStyle === null || rowStyle === void 0 ? void 0 : rowStyle.border} ${rowStyle.bg} ${rowStyle.text} rounded-2xl ${rowStyle.shadow} font-black text-xl transition-transform ${i in [0, 1, 2] ? 'group-hover:scale-110 group-hover:rotate-3' : ''}  duration-300">
                ${i + 1}
...
function gameClickHandler(game) {
    ...
    `;
        ...
        yield renderTableWidgets(leaderboard);
...
function renderTableWidgets(leaderboard) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        time.textContent = `${(leaderboard.leaderboard.reduce((accumulator, currentValue) => accumulator + currentValue.game_data.playtime_forever, 0)).toLocaleString()} ч.`;
        users.innerHTML = `
